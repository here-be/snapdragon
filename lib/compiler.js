...
Compiler.prototype = {
  ...
  /**
   * Define a non-enumberable property on the `Compiler` instance. Useful
   * in pluggins for adding methods to an
   *
   * ```js
  ...
  mapVisit: function(parent) {
    ...
    for (var i = 0; i < nodes.length; i++) {
      nodes[i] = this.visit(nodes[i]) || nodes[i];
    }
    return this;
  },
