...
Base.extend(Parser, {
  ...
  position: function() {
    ...
    return function(node) {
      node.position = new Position(start, self.source, self);
      return node;
    };
  },
  ...
  parse: function(str, options) {
    ...
    options = extend({source: 'string'}, options);
    this.source = options.source;
    this.original += str;
    this.input += str;

    var node;
    while (this.input.length && (node = (this.stashed() || this.advance()))) {
